...
impl TableHolder {
    pub fn new() -> TableHolder {
        let saturated_by_temperature_table = SteamTable::new(
            data::SATURATED_BY_TEMPERATURE_TABLE.to_string()
        ).unwrap();
    
        let saturated_by_pressure_table = SteamTable::new(
            data::SATURATED_BY_PRESSURE_TABLE.to_string()
        ).unwrap();
...
fn get_user_input() -> Result <Vec<String>, InteractiveError> {
    ...
    if io::stdout().flush().is_err() {
        return Err(
            InteractiveError::OutputError("Error writing to stdout!".to_string()));
    }
    ...
    if io::stdin()
        ...
        .is_err() {
            return Err(
                InteractiveError::InputError("Error reading user input!".to_string())
            );
        }
...
fn handle_user_input(user_input: Vec<String>, table_holder: &TableHolder) -> Result<InteractiveState, InteractiveError> {
...
fn query_saturated_steam(user_input: Vec<String>, table_holder: &TableHolder) -> Result<(), InteractiveError> {
...
fn query_saturated_steam_at_parameter(parameter_name: &str, parameter_value: f32, table_holder: &TableHolder) -> Result<(), InteractiveError> {
    let query_result = 
        if parameter_name == "temperature" {
            table_holder.saturated_by_temperature_table.get_values_at_point(parameter_value)
        } else {
            table_holder.saturated_by_pressure_table.get_values_at_point(parameter_value)
        };
    
    if let Err(query) = query_result {
        match query {
            Error::ValueOutOfRange(min, max) => {
                let err_str = format!("{} value should be between {} and {} ", parameter_name, min, max);
